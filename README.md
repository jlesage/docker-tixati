...
This project provides a Docker container for [Tixati](https://www.tixati.com).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of Tixati.
...
[Releases]: https://github.com/jlesage/docker-tixati/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/tixati/tags
...
container, it would proxy all HTTP requests for `tixati.domain.tld` to
...
server {
	...
	location /tixati/ {
		...
		# Needed when audio support is enabled.
		location /tixati/websockify-audio {
			proxy_pass http://docker-tixati/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Tixati integrated into Docker image. |
...
Any desktop notification generated by Tixati is forwarded to the
